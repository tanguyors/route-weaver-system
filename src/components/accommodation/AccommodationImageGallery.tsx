...
import { useAccommodationImagesData, AccommodationImage } from '@/hooks/useAccommodationImagesData';
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
interface AccommodationImageGalleryProps {
...
interface SortableImageProps {
  ...
  isFirst: boolean;
  onDelete: (image: AccommodationImage) => void;
...
const SortableImage = ({ image, isFirst, onDelete, onSetMain, isDeleting }: SortableImageProps) => {
  ...
  return (
    ...
    >
      <img
        src={image.image_url}
        alt={`Accommodation image ${image.display_order}`}
...
export const AccommodationImageGallery = ({ accommodationId, partnerId }: AccommodationImageGalleryProps) => {
  const {
    images,
    isLoading,
    uploadImages,
    ...
    remainingSlots,
    maxImages,
  } = useAccommodationImagesData(accommodationId, partnerId);

  const [deleteConfirm, setDeleteConfirm] = useState<AccommodationImage | null>(null);
  const [localImages, setLocalImages] = useState<AccommodationImage[]>([]);
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    disabled: !canAddMore || isUploading || !accommodationId,
  });
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center gap-2 text-muted-foreground bg-muted/50 p-4 rounded-lg">
            ...
            <p>Save the accommodation first to add photos.</p>
          </div>
        </CardContent>
      </Card>
    );
